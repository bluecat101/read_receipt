...
class ComfirmReciept(tk.Frame):
  ...
  def __init__(self,items,date,store): 
    ...
    functionFrame.configure(width=parentFrame.winfo_width(),height=30) # set Style
    functionFrame.pack()                                               # set position

    
    totalLabel=tk.Label(functionFrame,text="合計")      # Label for "TOTAL"
    totalLabel.place(relx=0.8)                         # set position
    self.totalNumLabel=tk.Label(functionFrame,text=0)  # Label for total 
    self.totalNumLabel.place(relx=0.9)                 # set position
    ...
    for i,element in enumerate(elements):  
      if i%8 == 3 or i%8 == 4:                                                           # for item price or item amount
        ...
        if isInt:                                                                        # integer or not
          ...
          total+=eachTotal                                                               # add each item total to total
        eachTotal=0
        isInt=True                                                                       # init variable
    self.totalNumLabel.configure(text=str(total)+"円")                                    # insert total num and "yesn"
